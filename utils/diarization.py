...
def words_per_segment(
    ...
    Args:
        ...
        fixed_margin (float): The fixed buffer time in seconds, bigger is better for noisy audio
        gap_scale_factor (float): The scale factor for the dynamic buffer, Dynamic buffer is calculated based on the previous and current segment
        .Bigger is better for noisy audio
...
def get_speakers(res_diarization: pyannote.core.Annotation) -> dict[str, int]:
    """Get the number of speakers in the diarization result"""
    speakers_ocurrences = [
        speaker for _, _, speaker in res_diarization.itertracks(yield_label=True)
    ]
    return {
        speaker: speakers_ocurrences.count(speaker) for speaker in speakers_ocurrences
    }
