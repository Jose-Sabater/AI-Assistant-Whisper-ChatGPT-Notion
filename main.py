...
class NotesAssistant:
    ...
    def video_to_audio(
        ...
        audio_file_path, audio = convert_video_to_audio(self.video_path, save_audio=save_audio, save_path=save_path, output_ext=output_ext)
    ...
    def text_to_summary(self, transcript:None, model:str ="gpt-3.5-turbo-0301",save_summary: bool = True):
        """Creates a summary out of a transcript using OpenAI ChatGPT API"""
        if transcript is None:
                transcript = self.transcript_path
        summary_path = summarize(self.transcript, model=model, save_summary=save_summary)
